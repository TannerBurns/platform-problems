...
def generate(random, pid, autogen_tools, n):
    ...
    template_path = path.join(generator_path, "login.html.template")
    rendered_template_path = path.join(generator_path, "login.html")
    ...
    homepage_link = autogen_tools.generate_resource_link(pid, "login.html", title="login page")

    return {
        "resource_files": {
            "public": [
                (rendered_template_path, "login.html"),
            ],
        },
        "static_files": {
        },
